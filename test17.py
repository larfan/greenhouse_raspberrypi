...
import MCP3008
import time, random, operator, copy, traceback, itertools, signal, sys
...
class realDB:
    def __init__(self):
        ...
        self.devicenames=['pump','fan','growlights','heaterelement']
    ...
    def devices(self,pointinprocess,widgidx,device,reason,reasonforpoweroff,element,duration):
        ...
        if pointinprocess == 'start': 
            ...
            #make query
            self.devicesquery = "INSERT INTO " +self.devicenames[device]+ " (reason, timestamp,duration_status,valuestart,valueend,aimed_at_value,reason_for_poweroff) VALUES (%s, %s,%s,%s, %s,%s, %s)"
    ...
    def messages(self,index,message,consequences):
        ...
        if message[:5]=='curre':         #entered when logging violation of time frame
            ...
            self.idquery="select IFNULL(MAX(id),0) from " +self.devicenames[self.startingdevice] #when db table has just been reset, IFNULL sets the NULL(in pyhton None) to 0, NUll is received because there are no rows yet in table
            ...
            self.messagesentry[1]=message
            self.messagesentry[2]=consequences
            self.messagesentry[3]=self.devicenames[self.startingdevice]+' '+str(self.tableindex[0][0]+1)
            ...
            for idx,element in enumerate(self.li):
                ...
                for self.data in self.onecheckintervallinstance(element,idx):           #this replaces while values.checkintervall(element[0],idx,element)!=True loop,necessary to get stable a variable of condition                  
                    ...
                    if element[4][0]==None and self.direction[0]!= None:                 #this guarantees that certain measurands don't get corrected at all, or only partially(like only raising the value)
                        #DB
                        DB.devices('start',[idx],self.direction[0],DB.measurandnames[element[0]]+' '+str(self.data),None,element,None)
                        ...
                        if self.direction[0]!= None:                        #it enters this loop, if it actually got a device to power up, in case of i.e lightintenistity too high, it doesn't
                            #write into DB
                            DB.devices('start',[idx],self.useddevice,DB.measurandnames[element[0]]+' '+str(self.data),None,element,None)
                    ...
                    if element[5][3] is not None:
                        if element[5][3][0]<element[5][3][1]:
                            if self.hour >=element[5][3][0] and self.hour <= element[5][3][1]:
                                ...
                                #DB
                                DB.messages(index,'currently in forbidden hours',DB.measurandnames[index]+' not corrected')
